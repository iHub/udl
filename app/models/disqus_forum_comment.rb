class DisqusForumComment < ActiveRecord::Base
	...
	has_many :comments, through: :tweet_answers, class_name: "Tagger::Answer", source: :answer
	...
	def self.create_self(comments)
		comments[:response].each do |comment|
			...
			forum.disqus_forum_comments.create(
				...
				author_message: comment[:message], author_created_at: comment[:createdAt], forum_id: comment[:id], forum_thread: comment[:thread], 
